		...
		<div class="col-md-6 pull-right">
			<div class="col-md-12 text-right">
				<ul class="list-unstyled invoice-details ">
					...
					<li>
						<strong>Invoice #:</strong> <?php echo isset($all_stocks_transaction[0]->stocks_transaction_date) ? date('Ymd',strtotime($all_stocks_transaction[0]->stocks_transaction_date)):'';?><?php echo isset($all_stocks_transaction[0]->stocks_transactions_id) ? $all_stocks_transaction[0]->stocks_transactions_id:'';?>
					</li>
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu" role="menu">
						<li role="presentation">
							<a tabindex="-1" role="menuitem" href="{{url('/inventory/purchase/invoice/download/pdf')}}" class=" ">
								PDF
							</a>
						</li>
						<li role="presentation">
				...
				<a target="_blank" href="{{url('/inventory/purchase/invoice/print')}}" class="btn  btn-purple">
			...
			<table class="table table-striped table-bordered table-hover">
				...
				<tbody>
					@if(isset($all_stocks_transaction) && !empty($all_stocks_transaction))
						<?php $stocks_total_bill= 0; ?>
						
						@foreach($all_stocks_transaction as $key => $stocks_transaction)
							<tr>
								<td>{{($key+1)}}</td>
								<td>{{$stocks_transaction->item_name}}</td>
								<td>{{$stocks_transaction->transaction_stocks_quantity}}</td>
								<td>{{number_format($stocks_transaction->stocks_quantity_rate,2,'.','')}}</td>
								<td>{{number_format($stocks_transaction->stocks_quantity_cost,2,'.','')}}</td>
							</tr>
							<?php $stocks_total_bill= $stocks_total_bill+$stocks_transaction->stocks_quantity_cost; ?>
						@endforeach
						<tr>
							<th colspan="4">Grand Total</th>
							<th class="text-center">{{$stocks_total_bill}}</th>
						</tr>
					@else
						<tr>
