...
<html lang="en">
  <head>
    <meta charset="utf-8">
    <!-- Title here -->
    <title><?php echo isset($page_title) ? $page_title:'Invoice'; ?></title>
    ...
    <main>
      <div id="details" class="clearfix">
        ...
        <div class="content_right">
          <p><span>Date: <?php echo isset($all_stocks_transaction[0]->stocks_transaction_date) ? date('j M, Y',strtotime($all_stocks_transaction[0]->stocks_transaction_date)):'';?></span></p> 
          <p><span>Bill No: <?php echo isset($all_stocks_transaction[0]->stocks_transaction_date) ? date('Ymd',strtotime($all_stocks_transaction[0]->stocks_transaction_date)):'';?><?php echo isset($all_stocks_transaction[0]->stocks_transactions_id) ? $all_stocks_transaction[0]->stocks_transactions_id:'';?></span></p>
        </div>
      </div>
      <div id="client">
          <div class="to"><?php echo isset($page_title) ? $page_title:'Invoice'; ?></div>                  
       </div>
      ...
      <div class="invoice-list">
        ...
        <?php 
                $stocks_total_bill= 0;
                foreach ($all_stocks_transaction as $key => $inventory_stocks) { ?>
                  <ul class="voucher_content">
                    
                    <li  style="width:50px; text-align:center;font-weight:bold;"><?php echo ($key+1); ?></li>
                    <li  style="width:325px;padding-left:10px;"><strong><?php echo isset($inventory_stocks->item_name) ? $inventory_stocks->item_name:'';?></strong></li>
                    ...
                    <?php $stocks_total_bill= $stocks_total_bill+$inventory_stocks->stocks_quantity_cost; ?>
                  </ul>
           <?php } ?>
      </div>
      <div class="total_block">
        <ul>
          ...
          <li style="width:135px;text-align:center;font-weight:bold;">Tk <?php echo isset($stocks_total_bill) ? number_format($stocks_total_bill,2,'.','' ):'';?> </li>
        </ul>
      </div>
      <div class="amounts_in_word">
        <p><strong>Amount In words:</strong>
        <?php
          if(is_numeric($stocks_total_bill)){
            $amount_words = \App\System::ConvertNumberToWords($stocks_total_bill);
