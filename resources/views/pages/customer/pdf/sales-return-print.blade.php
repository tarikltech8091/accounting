...
<html lang="en">
  <head>
    <meta charset="utf-8">
    <!-- Title here -->
    <title> {{isset($page_title)? $page_title:'Sales Return' }} </title>
    ...
    <main>
      <div id="details" class="clearfix">
        ...
        <div class="content_right">
          ...
          <p><span>Delivered Date: {{isset($ltech_sales_orders->order_delivered_customer_date)?($ltech_sales_orders->order_delivered_customer_date):''}}</span></p>
        </div>
      </div>
      <div id="client">
           ...
           @if(isset($sales_return_all_data['return_confirm_entry_field']) && $sales_return_all_data['return_confirm_entry_field'] !=0)
            ...
            @for($i=1;$i<=$sales_return_all_data['return_confirm_entry_field'];$i++)
              ...
              <ul class="voucher_content">
                <li  style="width:50px; text-align:center;font-weight:bold;">{{$i}}</li>
                <li  style="width:325px;font-weight:bold;"><span>{{$sales_return_all_data['return_quantity_name_'.$i]}}</span></li>
                <li  style="width:70px; text-align:center;font-weight:bold;">{{$sales_return_all_data['return_quantity_'.$i]}}</li>
                 <li  style="width:115px;text-align:center;font-weight:bold;">{{$sales_return_all_data['return_quantity_rate_'.$i]}}</li>
                <li  style="width:135px; text-align:center;font-weight:bold;">{{$sales_return_all_data['return_quantity_'.$i]*$sales_return_all_data['return_quantity_rate_'.$i]}}</li>
              </ul>
      ...
      <div class="total_block">
        <ul>
          ...
          <li style="width:135px;text-align:center;font-weight:bold;">{{isset($total)?($total):' 00.00 '}} </li>
        </ul>
      </div>
      ...
      <div class="amounts_in_word">
        <p><strong>Amount In words:</strong> 
          <?php
            if(is_numeric($total)){
              $amount_words = \App\System::ConvertNumberToWords($total);
