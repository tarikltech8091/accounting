...
<div class="col-md-12">		
	...
	<div class="row">
		...
		<div class="col-md-6">
			<h2>{{isset($company_details->company_name)? $company_details->company_name :''}}</h2>
			<p>{{isset($company_details->company_address)? $company_details->company_address :''}}</p>
		</div>
		<div class="col-md-6 text-right">
			<a target="_blank" href="{{url('/reports/balance-sheet/print/from-'.$search_from.'/to-'.$search_to.'/cid-'.$cost_center)}}" class="btn  btn-purple">
				Print <i class="fa fa-print"></i>
			</a>
			<a href="{{url('/reports/balance-sheet/pdf/from-'.$search_from.'/to-'.$search_to.'/cid-'.$cost_center)}}" class="btn btn-green ">
...
<hr>
    <div class="row">
        <form method="get" action="{{url('/reports/balance-sheet')}}">
          ...
          <div class="col-md-2" style="margin-top:22px;">
            <div class="form-group">
              <input type="submit" class="btn btn-primary" data-toggle1="tooltip" title="Search" value="View">
            </div>
          </div>
        </form>
    </div>
			...
			<?php 
			 ...
			 ?>
				...
				<tr>
					...
					<td colspan="2" align="right"></td>
					<?php
						$grand_total_loans_balance=(isset($grand_total_loan_liability)? $grand_total_loans_balance:0);
						$grand_total_loans_balance=\App\Report::MakePositiveData($grand_total_loans_balance);  ?>
					<th align="left">{{isset($grand_total_loan_liability)?$grand_total_loan_liability :0}}</th>
				</tr>

				@if(!empty($loan_liability) && count($loan_liability)>0)
					@foreach($loan_liability as $key => $account)
						<tr>
							<td colspan="2" style="padding-left:50px;">{{$account['particular_name']}}</td>
							<?php 
								// $total_loans_balance=$account['debit']-$account['credit'];
								// $total_loans_balance=\App\Report::MakePositiveData($total_loans_balance);
								$total_loans_balance=\App\Report::MakePositiveData($account['paritcular_total']);
							?>
				...
				<tr>
					...
					<?php
						...
						$current_profits=0;
						$opening_balance_amount=\App\Report::MakePositiveData($opening_balance);
						$current_profits=\App\Report::MakePositiveData($current_profit);
					?>
				
				<tr>
					<td colspan="2" style="padding-left:50px;">Opening Balance</td>
					<td colspan="2" align="left">{{isset($opening_balance_amount)? $opening_balance_amount :0.00}}</td>
				</tr>
				<tr>
		...
		<table class="table table-striped">
			...
			<tbody>
				...
				<tr>
					...
					<td colspan="2" align="right"></td>
					<?php 
						$total_fixed_assets=(isset($grand_total_fixed_assets)?$grand_total_fixed_assets : 0);
						$total_fixed_assets=\App\Report::MakePositiveData($total_fixed_assets);  
					?>
				...
				@if(!empty($fixed_assets) && count($fixed_assets)>0)
					@foreach($fixed_assets as $key => $accounts)
							...
							<?php 
								...
								$fixed_assets_balance=$accounts['paritcular_total'];
							?>
				...
				@if(count($investment_accounts)>0)
					@foreach($investment_accounts as $key => $accounts)
							...
							<td colspan="2" style="padding-left:50px;">{{isset($accounts['particular_name'])?$accounts['particular_name']:''}}</td>
							<?php 
								// $investment_balance=$accounts['debit']-$accounts['credit'];
								// $investment_balance=\App\Report::MakePositiveData($investment_balance);
...
<!-- 
				...
				@if(!empty($stock_in_hand_total) && count($stock_in_hand_total)>0)
				<tr>
				<?php 
					// $stock_in_hand_total=\App\Report::MakePositiveData($stock_in_hand_total);
				?>
					<td colspan="2" style="padding-left:50px;">Stock-in-hand</td>
					<td colspan="2" align="left">{{$stock_in_hand_total}}</td>
