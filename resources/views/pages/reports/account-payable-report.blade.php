  ...
  <div class="col-md-12">
    ...
    <?php $company_details=\DB::table('company_details')->latest()->first(); ?>
      ...
      <div class="panel panel-heading" align="center">
        <strong> Account Payable Report </strong>
        <a href="{{url('/account-payable/report/pdf/from-'.(isset($search_from)? $search_from : '').'/to-'.(isset($search_to)? $search_to :'').'/ccid-'.(isset($cost_center_id)? $cost_center_id :''))}}" class="btn btn-xs btn-green tooltips pull-right" data-toggle1="tooltip" title="Purchase PDF Download" style="margin-left:10px;"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>

        <a target="_blank" href="{{url('/account-payable/report/print/from-'.(isset($search_from)? $search_from : '').'/to-'.(isset($search_to)? $search_to :'').'/ccid-'.(isset($cost_center_id)? $cost_center_id :''))}}" class="btn btn-xs btn-green tooltips pull-right" data-toggle1="tooltip" title="Purchase PDF Print" ><i class="fa fa-print" aria-hidden="true"></i></a>
      </div>


      <div class="panel-body">
        <div class="row">
          <form method="get" action="{{url('/account-payable/balance/report')}}">
