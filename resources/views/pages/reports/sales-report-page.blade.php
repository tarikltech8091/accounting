  ...
  <div class="col-md-12">
    ...
    <?php $company_details=\DB::table('company_details')->latest()->first(); ?>
      ...
      <div class="panel panel-heading" align="center">
        ...
        <a href="{{url('/sales/report/pdf/from-'.(isset($search_from)? $search_from : '').'/to-'.(isset($search_to)? $search_to :'').'/ccid-'.(isset($cost_center_id)? $cost_center_id :''))}}" class="btn btn-xs btn-green tooltips pull-right" data-toggle1="tooltip" title="Sales Report  Download" style="margin-left:10px;"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>

        <a target="_blank" href="{{url('/sales/report/print/from-'.(isset($search_from)? $search_from : '').'/to-'.(isset($search_to)? $search_to :'').'/ccid-'.(isset($cost_center_id)? $cost_center_id :''))}}" class="btn btn-xs btn-green tooltips pull-right" data-toggle1="tooltip" title="Sales Report Print" ><i class="fa fa-print" aria-hidden="true"></i></a>
      </div>


      <div class="panel-body">
        <div class="row">
          <form method="get" action="{{url('/sales/balance/report')}}">
        ...
        <div class="table-responsive cost_list posting_list">
          <table class="table table-hover table-bordered table-striped nopadding">
            ...
            ?>
               ...
               @if(isset($sales_goods_data) && count($sales_goods_data)>0)
               @foreach($sales_goods_data as $key => $list)
                <tr>
                  <td colspan="2" class="text-center">{{$list['finish_goods_name']}}</td>
                  <?php
