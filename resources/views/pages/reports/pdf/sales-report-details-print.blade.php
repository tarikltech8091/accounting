...
.content_right span {
  ...
  <!-- <body > -->
    ...
    <?php $company_details=\DB::table('company_details')->latest()->first(); ?>
      ...
      <div id="client">
          <div class="to"><strong> Sales Report of {{isset($type)? $type :''}} </strong></div>                  
       </div>
          ...
          if(count($details_list)>0){
            ...
            foreach ($details_list as $key => $list) {
              if($list->journal_particular_amount_type=='debit'){ 
                ...
                $total_debit=$total_debit+$list->journal_particular_amount;
                ?>
                <ul class="voucher_content">
                  <li  style="width:100px; "><?php echo isset($list->journal_date)? $list->journal_date :'';?></li>
                  <li  style="width:425px;"><span><?php echo isset($list->journal_particular_name)? $list->journal_particular_name:'';?></span><br/>
                  <i style="margin-left:10px; font-size:12px"><?php echo !empty($list->journal_particular_naration)? '('.$list->journal_particular_naration.')':'';?></i></li>
                  <li  style="width:170px;text-align:center;font-weight:bold;"><?php echo isset($list->journal_particular_amount)? number_format($list->journal_particular_amount,2,'.',''):'';?></li>
                </ul>
            <?php }
      ...
      <div class="total_block">
        <ul>
          ...
          <li style="width:170px;text-align:center;font-weight:bold;">Tk <?php echo isset($total_debit) ? number_format($total_debit,2,'.',''):'';?></li>
        </ul>
      </div>
